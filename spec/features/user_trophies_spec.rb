feature 'a user dashboard page trophies' do
  ...
  context 'after completing a quiz' do
    before(:each) do
      visit("/quiz/#{@quiz[:quiz].id}")
    end

    context 'and getting 100% (1 mark)' do
      before(:each) do
        choose @quiz[:answer1_1].id
        click_on 'Mark'
        visit_dashboard
      end

      it 'should show the Full Marks trophy.' do
        expect(page).to have_content('Trophies: 1')
        expect(page).to have_content('Full Marks')
        expect(page).to have_content('Complete the My quiz quiz with a score greater or equal to 1')
      end
    end

    context 'and getting 0 marks' do
      before(:each) do
        choose @quiz[:answer1_2].id
        click_on 'Mark'
        visit_dashboard
      end
      ...
      it 'should show the Full Marks trophy.' do
        expect(page).to have_content('Trophies: 0')
        expect(page).not_to have_content('Full Marks')
        expect(page).not_to have_content('Complete the My quiz quiz with a score greater or equal to 1')
      end
    end
